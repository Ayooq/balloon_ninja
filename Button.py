...
class Button(Sprite):
    ...
    def __init__(self, screen, x_pos, y_pos, settings, msg):
        ...
        self.width, self.height = settings.button_width, \
        ...
        self.font = pygame.font.SysFont(
            settings.button_font, settings.button_font_size
        )
        self.msg = msg

        # Button message only needs to be pressed once, not on every blit:
        self.prep_msg()

    def prep_msg(self):
        # Turn msg into image that can be rendered:
        self.msg_image = self.font.render(
            self.msg, True, self.settings.button_text_color
        )
        # Determine offset to center text on the button:
        self.msg_x = self.x_position + (
            self.width - self.msg_image.get_width()
        ) / 2
        self.msg_y = self.y_position + (
            self.height - self.msg_image.get_height()
        ) / 2
