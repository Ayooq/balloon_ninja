...
class Balloon(Sprite):
    ...
    def __init__(self, screen, settings):
        ...
        self.image = pygame.image.load(
            'green_balloon_small.png'
        ).convert_alpha()
        self.image_w, self.image_h = self.image.get_size()
        self.speed = uniform(0.75, 1.25) * settings.balloon_speed

        self.x_position = randint(
            self.image_w / 2, self.screen.get_width() - self.image_w / 2
        )
        max_offset = min(
            settings.batch_size * self.image_h, self.screen.get_height()
        )
        self.y_position = self.screen.get_height() + \
            self.image_h / 2 + randint(0, max_offset)
