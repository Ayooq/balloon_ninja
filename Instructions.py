...
class Instructions(Sprite):
    ...
    def __init__(self, screen, settings):
        ...
        self.instr_lines = ['Move your mouse to swipe the sword ' \
            'back and forth.']
        ...
        self.instr_lines.append('Letting kittens live earns several balloons' \
            '\' worth of points.')
        self.instr_lines.append('Killing a kitten loses several balloons' \
            '\' worth of points.')

        # The instruction message only needs to be prepped once,
        # not on every blit:
        self.prep_msg()
    ...
    def prep_msg(self):
        ...
        for index, line in enumerate(self.instr_lines):
            self.msg_images.append(
                self.font.render(
                    line, True, self.text_color
                )
            )
            self.msg_x.append(
                self.settings.screen_width / 2 - self.font.size(line)[0] / 2
            )
            self.msg_y.append(
                y_position + index * self.font.size(line)[1]
            )
    def blitme(self):
        for msg_x, msg_y, msg_image in zip(
                self.msg_x, self.msg_y, self.msg_images
        ):
