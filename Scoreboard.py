...
class Scoreboard(Sprite):
    ...
    Creates a panel to print player's game statistics.
    """
    def __init__(self, screen, settings):
        ...
        self.sb_height, self.sb_width = settings.scoreboard_height, \
        ...
        self.x_score_position, self.y_score_position = \
            self.screen.get_width() - 200, 10.0
    ...
    def prep_scores(self):
        ...
        self.popped_image = self.font.render(
            self.popped_string, True, self.text_color
        )
        self.set_ratio_string()
        self.popped_ratio_image = self.font.render(
            self.popped_ratio_string, True, self.ratio_text_color
        )
        self.points_string = 'Points per Balloon: ' + str(
            self.settings.points_per_balloon
        )
        self.points_image = self.font.render(
            self.points_string, True, self.text_color
        )
        self.score_string = 'Score: ' + format(self.score, ',d')
        self.score_image = self.font.render(
            self.score_string, True, self.text_color
        )
    ...
    def set_ratio_string(self):
        if self.popped_ratio == 1.0:
            ...
            self.popped_ratio_string = 'Pop rate: {0:.3}%'.format(
                self.popped_ratio * 100.0
            )
    ...
    def blitme(self):
        ...
        self.screen.blit(
            self.popped_image, (
                self.x_popped_position, self.y_popped_position
            )
        )
        self.screen.blit(
            self.popped_ratio_image, (
                self.x_ratio_position, self.y_ratio_position
            )
        )
        self.screen.blit(
            self.points_image, (
                self.x_points_position, self.y_points_position
            )
        )
        self.screen.blit(
            self.score_image, (
