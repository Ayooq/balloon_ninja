...
class Engine():
    ...
    def release_batch(self):
        for x in range(self.settings.batch_size):
            self.spawn_balloon()
    ...
    def check_balloons(self, time_passed):
        ...
        if self.scoreboard.balloons_popped > 0 or \
                self.scoreboard.balloons_missed > 0:
            self.scoreboard.popped_ratio = \
                float(self.scoreboard.balloons_popped) / (
                    self.scoreboard.balloons_popped + \
                    self.scoreboard.balloons_missed
                )
            if self.scoreboard.popped_ratio < self.settings.min_popped_ratio:
                # Set game active to False, empty the list of balloons,
                # and increment games_played:
                self.settings.game_active = False
                self.settings.games_played += 1

    def check_kittens(self, time_passed):
        # Find any kittens that've either been killed or survived:
        for kitten in self.kittens:
            ...
            if kitten.y_position < -kitten.image_h / 2 \
                    + self.settings.scoreboard_height:
    ...
    def spare_kitten(self, kitten):
        ...
        self.scoreboard.score += self.settings.kitten_score_factor * \
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.sword.rect.collidepoint(mouse_x, mouse_y):
                    ...
                    # Empty list of balloons, initialize scoreboard
                    # and game parameters, and make game active:
                    del self.balloons[:]
